...
$this->title = Yii::t('frontend', 'Warning Ads') . ' | OLX';
          ...
          <div class="row">
            <div class="col-sm-12">
              <div class="section-title my_ads_title ">
                <h2><?php echo Yii::t('frontend', 'Warning Ads')?></h2>
              </div>
              <div class="featured_slider h_products table-responsive">
				  ...
				  <table class="table_my_ads">
					  ...
					  <?php if(!empty($warning_ads)){?>  
					  <tbody>
					<?php foreach($warning_ads as $key=>$myad){?>
					  	<tr>
							...
							<?php  $ad_warning =  AppHelper::getAdWarning($myad['id']);
							$warning  = $ad_warning->message;
							?>
							<td class="item_status"><a href="javascript:void(0)" data-toggle="modal" data-target="#warningModal<?php echo$myad['id'];?>"><span class="label status_label<?php echo$myad['id'];?>">View &nbsp;<?php echo $myad['status']?></span></a></td>
							<td class="action">
								<a href="<?php echo $baseurl.'/user/edit-ad/'.$myad['id']; ?>"><i class="fa fa-pencil"></i> <?php echo Yii::t('frontend', 'Edit')?></a><br>
								<a class="hide item-js-hide" attr-id="<?php echo $myad['id']?>" href="javascript:void(0)" ><i class="fa fa-eye-slash"></i> <?php echo Yii::t('frontend', 'Hide')?></a>
							</td>
						</tr>
						
						
						
						<div class="modal" tabindex="-1" role="dialog" id="warningModal<?php echo$myad['id'];?>">
...
<?php
   $jss = "$(document).on('click', '.hide', function() {
    var pro_id = $(this).attr('attr-id');
   
   if(pro_id){
	   	 $.ajax({
              url: '$baseurl/user/hide-ad',
              type: 'POST',
               data: {pro_id: pro_id}, 
               success: function(data) {
                var data = JSON.parse(data);
                $('.status_label'+pro_id).html(data.status);
