...
jQuery(function($) {
    ...
    var update_staff_choice = null,
        ...
        $new_category_link = $('.category_submit');
    ...
    $new_category_link.on('click', function(e) {
        e.preventDefault();
        var data = $('#new-category-form').serialize();
        var ajaxurl = $('#new-category-form').attr("action");
        $('#new-category-form').find('.confirm').addClass('bookme-progress');
    ...
    $(document)
        ...
        .on('click', '.quickad-category-item', function(e) {
            ...
            console.log($clicked.data('category-id'))

            if (typeof $clicked.data('category-id') === 'undefined') {
                $ajaxurl=baseurl+'/categories/get-sub-cat';
            }
            else{
                $ajaxurl=baseurl+'/categories/get-sub-cat?category_id='+$clicked.data('category-id');
            }

            $.get($ajaxurl, function(response) {
                if ( response != 0 ) {
                    $('.quickad-category-title').text($clicked.find('.displayed-value').text());
                    $('#ab-services-list-main').html(response);
                    $('#quickad-delete').css('display','inline-block');
                    $('.new-subcategory').css('display','inline-block');
                }else{
                    //alert($clicked.find('.displayed-value').text());
                    $('.quickad-category-title').text($clicked.find('.displayed-value').text());
                    $('#ab-services-list-main').html(response);
                    $('#quickad-delete').css('display','none');
       ...
       /* .on('click', '.quickad-js-edit', function(e) {
        ...
        .on('click', '.quickad-cat-lang-edit', function(e) {
            ...
            var ajaxurl=baseurl+'/categories/get-lang-translation-form';
            $.post(ajaxurl, data, function(response) {
                if(response != "") {
...
$('#modal_LangTranslation').on('click', '#saveEditLanguage', function() {
    ...
    var ajaxurl=baseurl+'/categories/edit-lang-translation';
