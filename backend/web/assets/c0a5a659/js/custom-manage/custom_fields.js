jQuery(function($) {
    ...
    $fields.on('click', '.quickad-js-delete', function(e) {
        ...
        ajaxurl=baseurl+'/custom-fields/delete-custom-fields';
    ...
    $fields.on('click', '.quickad-option-delete', function(e) {
        ...
        ajaxurl=baseurl+'/custom-fields/delete-custom-option';
        $.post(ajaxurl, data, function(response) {
            if(response == 1) {
                $item.fadeOut('fast', function() { $item.remove(); });
                quickadAlert({success: ['Successfully deleted.']});
    ...
    $fields.on('click', '.quickad_language_translation', function(e) {
        ...
        var id = $(this).data('custom-field-id'),
            type = "custom-field",
            data = { id: id, cat_type: type};

        var ajaxurl=baseurl+'/custom-fields/get-customfield-lang-translation-form';
        $.post(ajaxurl, data, function(response) {
            if(response != "") {
    ...
    $('#modal_LangTranslation').on('click', '#saveCustomEditLanguage', function() {
        var $this = $(this),
        ...
        $("#modal_LangTranslation .label_code").map(function(){
            $name = $(this).val();
        ...
        $('#saveCustomEditLanguage').addClass('bookme-progress');

        var data = {id: id, trans_lang: $trans_lang, trans_name: $trans_name };
        var ajaxurl=baseurl+'/custom-fields/edit-customfield-lang-translation-form';
        $.ajax({
            type: "POST",
            data: data,
            url: ajaxurl,
            success: function(response){
                ...
                $('#saveCustomEditLanguage').removeClass('bookme-progress');
            }
        });
    ...
    $fields.on('click', '.quickad_itmes_translation', function(e) {
        ...
        var ajaxurl=baseurl+'/custom-fields/get-option-lang-translation-form';
        $.post(ajaxurl, data, function(response) {
            if(response != "") {
    ...
    $('#Options_LangTranslation').on('click', '#saveEditLanguage', function() {
        ...
        var data = { id: id, cat_type: type, value: $selected };
        var ajaxurl=baseurl+'/custom-fields/edit-option-lang-translation-form';
        $.ajax({
            type: "POST",
            data: data,
            url: ajaxurl,
    ...
    $('#ajax-send-custom-fields').on('click', function(e) {
        ...
        ajaxurl=baseurl+'/custom-fields/custom-field-saved';
        $.ajax({
            type      : 'POST',
            url       : ajaxurl,
            xhrFields : { withCredentials: true },
            data      : { csrf_token : 12232412, fields: JSON.stringify(data), cf_per_service: $cf_per_service.val() },
            complete  : function() {
                ladda.stop();
    ...
    function addField(type, id, label, required, services, translation) {
        ...
        var count = $('#quickad-custom-fields').children().length;
        var optionname= 'option['+count+']';
        var optionreqname= 'optionreq['+count+']';
        ...
        $new_field
            ...
            .end()
            .find('.quickad-label').attr('name',optionname)
            .end()
        ...
        $new_field.find('ul.quickad-items').sortable({
            ...
            update : function( event, ui ) {
                ...
                $.ajax({
                    type : 'POST',
                    url  : ajaxurl,
                    data : {field_id: field_id, position: data},
                    success: function (response, textStatus, jqXHR) {
                        // Remove Ads item from DOM.
                        if(response != 0) {
    ...
    function updateServiceButton($holder) {
        ...
        if (service_checked == 0) {
            ...
            $holder.find('.quickad-js-check-entity').prop('checked',
                (service_checked == $holder.find('.quickad-js-check-sub-entity').length));
