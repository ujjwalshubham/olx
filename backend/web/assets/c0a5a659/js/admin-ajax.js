...
function refreshAjaxTable(){
    console.log('fgfdgdfgdffd');
    $.pjax({container:'#pjax-grid',push:false,replace:false,data:$(this).serialize()});
    $.slidePanel.hide();
    console.log('fgfd');
    ...
    function showResponse(data, statusText, xhr, $form) {
        ...
        if (data == 0) {
            ...
            if (data.status == "success") {
                ...
                errorslist=data.errormsg;
                $('div.help-block').remove();
                $('div.form-group').removeClass('has-error');
                $.each(errorslist, function(key, val) {
                    $("#"+key).next('div.help-block').remove();
